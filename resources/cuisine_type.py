...
cuisine_type_ns = Namespace('cuisine_type', description='CuisineType related operations')
cuisine_types_ns = Namespace('cuisine_types', description='CuisineTypes related operations')

cuisine_type_schema = CuisineTypeSchema()
cuisine_type_list_schema = CuisineTypeSchema(many=True)

cuisine_type = cuisine_types_ns.model('CuisineType', {
...
class CuisineType(Resource):
    ...
    @is_admin
    def delete(self, id):
        cuisine_type_data = db.session.query(CuisineTypeModel).filter(CuisineTypeModel.id == id).first()
        if cuisine_type_data:
